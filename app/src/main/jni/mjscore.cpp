...
//成立した役名一覧
void MJScore::Get_Yakuname(jstring s[]){
#if 0
	for(int i=0;!YakuName[i].empty();i++)
		...
		for(int i=0;YakuName[i] != (jstring)"" ;i++)
#endif
		s[i]=YakuName[i];
}
...
MJScore::MJScore()
{
	...
	YakuNameTable[TENHOU]=(jstring)"天和";
	YakuNameTable[TIIHOU]=(jstring)"地和";
	YakuNameTable[NAGASHIMANGAN]=(jstring)"流し満貫";
	YakuNameTable[KOKUSHI]=(jstring)"国士無双";
	YakuNameTable[KOKUSHI13]=(jstring)"国士無双13面待";
	YakuNameTable[TYUUREN]=(jstring)"九連宝燈";
	YakuNameTable[TYUUREN9]=(jstring)"九連宝燈9面待";
	YakuNameTable[DAISUUSII]=(jstring)"大四喜";
	YakuNameTable[SYOUSUUSII]=(jstring)"小四喜";
	YakuNameTable[SUUANKOUTANKI]=(jstring)"四暗刻単騎待";
	YakuNameTable[SUUANKOU]=(jstring)"四暗刻";
	YakuNameTable[DAISANGEN]=(jstring)"大三元";
	YakuNameTable[TINROUTOU]=(jstring)"清老頭";
	YakuNameTable[RYUUIISOU]=(jstring)"緑一色";
	YakuNameTable[SUUKANTU]=(jstring)"四槓子";
	YakuNameTable[TUUIISOU]=(jstring)"字一色";

	YakuNameTable[SANKANTU]=(jstring)"三槓子 2";
	YakuNameTable[RIICHI]=(jstring)"立直 1";
	YakuNameTable[DOUBLERIICHI]=(jstring)"ダブル立直 2";
	YakuNameTable[IPPATU]=(jstring)"一発 1";
	YakuNameTable[TANYAO]=(jstring)"断幺九 1";
	YakuNameTable[PINFU]=(jstring)"平和 1";
	YakuNameTable[IIPEIKOU]=(jstring)"一盃口 1";
	YakuNameTable[SANANKOU]=(jstring)"三暗刻 2";
	YakuNameTable[TOITOI]=(jstring)"対々和 2";
	YakuNameTable[SANSYOKUDOUKOU]=(jstring)"三色同刻 2";
	YakuNameTable[RYANPEIKOU]=(jstring)"二盃口 3";
	YakuNameTable[TIITOITU]=(jstring)"七対子 2";
	YakuNameTable[HONROUTOU]=(jstring)"混老頭 2";
	YakuNameTable[HAKU]=(jstring)"飜牌(白) 1";
	YakuNameTable[HATU]=(jstring)"飜牌(発) 1";
	YakuNameTable[TYUN]=(jstring)"飜牌(中) 1";
	YakuNameTable[BAKAZE]=(jstring)"飜牌(場風) 1";
	YakuNameTable[JIKAZE]=(jstring)"飜牌(自風) 1";
	YakuNameTable[SYOUSANGEN]=(jstring)"小三元 2";
	YakuNameTable[HAITEITUMO]=(jstring)"海底撈月 1";
	YakuNameTable[HOUTEIRON]=(jstring)"河底撈魚 1";
	YakuNameTable[TYANKAN]=(jstring)"槍槓 1";
	YakuNameTable[RINSYAN]=(jstring)"嶺上開花 1";
	YakuNameTable[TUMO]=(jstring)"門前清模和 1";

	YakuNameTable[SANSYOKUDOUJYUN]=(jstring)"三色同順";
	YakuNameTable[ITTUU]=(jstring)"一気通貫";
	YakuNameTable[TYANTA]=(jstring)"全帯幺";
	YakuNameTable[JYUNTYANTA]=(jstring)"純全帯幺";
	YakuNameTable[HONITU]=(jstring)"混一色";
	YakuNameTable[TINITU]=(jstring)"清一色";

	YakuNameTable[DORA]=(jstring)"ドラ";
	YakuNameTable[URADORA]=(jstring)"裏ドラ";
	YakuNameTable[AKADORA]=(jstring)"赤ドラ";
}
...
int MJScore::Check_Haino(){
	...
	for(i=0;i<=37;i++){
			...
			if(TehaiNormalizeMarge[i]<0 || TehaiNormalizeMarge[i]>4){
				 return -1;
			}
...
void MJScore::Tehai_Normalize(){
	...
	for(int i=0;Fuuro[i]&&i<=12;i+=4){
		if((Fuuro[i]==PON)||(Fuuro[i]==AKA_PON))
		{
	...
	for(int i=0;i<=37;i++){
		// printf("test_after no %d\n",TehaiNormalizeMarge[i]);
	}
...
void MJScore::Run()
{
	...
	if(Agarihai==0 || Tehai[Agarihai]==0){
	...
	//鳴きリーチしている：エラーコード=5
	if(Is_FuuroWithoutAnkan() && Riichi){
	...
	memset(Yaku,false,sizeof(Yaku));
	Fan=Fu=0;
	for(int i=0;i<20;i++){YakuName[i]=(jstring)"";}
...
void MJScore::MentuKiriwake()
{
	...
	if(Fuuro[i]!=0){
		...
		else if(Fuuro[i]==PON||Fuuro[i]==AKA_PON){
...
bool MJScore::Is_Tanyao()
{
	...
	for(i=0;Fuuro[i]&&i<20;i+=4){
		if(Fuuro[i+1]>30||Fuuro[i+1]%10==1||Fuuro[i+1]%10==9||
			Fuuro[i+1]==7&&(Fuuro[i]==CHII || Fuuro[i]==AKA_CHII))
			return false;
	}
	return true;
}
...
bool MJScore::Is_Sansyokudoukou()
{
	...
	memset(chk,0,sizeof(chk));
	for(int i=0;i<10;i+= 2){
		if (kiriwake[i]==PON||kiriwake[i]==ANKO ||kiriwake[i]==ANKAN ||kiriwake[i]==KAKAN ||kiriwake[i]==MINKAN||kiriwake[i]==AKA_PON)
		{
			chk[kiriwake[i+1]]++;
		}
	}
...
void MJScore::NormalSyanten()
{
	...
	Nsyan=syanten_normal-Fuurosuu*2+sini_jihai; //最終的な結果
}
